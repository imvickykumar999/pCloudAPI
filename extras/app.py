...
def collection():
    if request.method == 'POST':
        ...
        if 'collection_file' in request.files:
            ...
            try:
                ...
                return render_template_string(HTML_TEMPLATE_COLLECTION, 
                                               requests_data=requests_data,
                                               header_count=1)
            except Exception as e:
                return render_template_string(HTML_TEMPLATE_COLLECTION, error=f"Error parsing collection: {e}", header_count=1)
    ...
    return render_template_string(HTML_TEMPLATE_COLLECTION, response=None, header_count=1)
...
<div class="container">
    <a href="/collection" style="text-decoration: none; color: #ff6f00;">
        <h2 style="text-align: center;">Go to API Collection</h2>
    </a>
...
<head>
    ...
    <script>
        ...
        function copyToClipboard() {
            var content = document.getElementById("responseContent").innerText;
...
<div class="container">
    ...
    <a href="/" style="text-decoration: none; color: #ff6f00;">
            <h2 style="text-align: center;">Go to API Tester</h2>
    </a>
    ...
    <form id="form_{{ loop.index }}" method="POST" enctype="multipart/form-data">
        <div class="one-block">
            <button type="button" class="collapsible">{{request.method}} - {{request.name}}</button>
            <div class="content">
    ...
    <div class="response-container">
        ...
        <div style="position: relative;">
            <pre id="responseContent">{{ response }}</pre>
            <button onclick="copyToClipboard()" style="position: absolute; top: 10px; right: 10px; background: none; border: none; cursor: pointer;">
